#!/usr/bin/env bash
# polymath-connector-github-actions Stop hook
#
...
case "$conclusion" in
  failure|cancelled|timed_out)
    echo "[polymath-connector-github-actions] Latest CI run \"$name\" on \`$branch\` ended in $conclusion. $url — fetch logs via the github MCP server or re-run with workflow.dispatch?"
    ;;
esac
