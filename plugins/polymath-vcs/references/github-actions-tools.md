...
server shipped by `polymath-vcs`. Installing this plugin gives
...
Same PAT as `polymath-vcs`. Add:

- `workflows: read+write` for dispatch + rerun.
- `actions: read` for log access.

If the same PAT serves both connectors, set it once via `polymath-vcs`'s `userConfig.githubToken`.
