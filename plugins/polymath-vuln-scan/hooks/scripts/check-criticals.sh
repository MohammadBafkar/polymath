...
if [[ -n "$criticals" && "$criticals" -gt 0 ]]; then
  echo "[polymath-vuln-scan] $criticals open critical vulnerability(ies) in this project (per cached snyk test). Consider re-running \`snyk test\` or fetching via the snyk MCP before shipping."
fi
