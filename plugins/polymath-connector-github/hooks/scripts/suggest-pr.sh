#!/usr/bin/env bash
# polymath-connector-github Stop hook
#
...
if [[ -n "$upstream" ]]; then
  ...
  if [[ "$ahead" -gt 0 ]]; then
    ...
    echo "[polymath-connector-github] $ahead unpushed commit(s) on \`$branch\`. Ready to push and open a PR via the github MCP server?"
    exit 0
  fi
fi
