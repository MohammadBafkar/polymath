#!/usr/bin/env bash
# polymath-connector-github Stop hook (GitHub Actions diagnostics)
#
...
case "$conclusion" in
  failure|cancelled|timed_out)
    echo "[polymath-connector-github] Latest CI run \"$name\" on \`$branch\` ended in $conclusion. $url — fetch logs via the github MCP server or re-run with workflow.dispatch?"
    ;;
esac
