...
- A workflow invokes `polymath-deprecation:migration-guide`.

This writes the *consumer upgrade steps*. It is not the deprecation timeline/policy (`polymath-deprecation:deprecation-plan`), the customer-facing sunset notice (`polymath-content:write-sunset-notice`), or a runtime/version migration of your own codebase (the `migrateLanguageVersion` workflow).
