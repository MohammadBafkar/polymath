...
`review-migration` and `audit-pg-config` were folded in from the former
`polymath-infra-postgres` plugin: migration safety lives next to `db-schema`
and `migration-plan` rather than in a separate one-decision install.
