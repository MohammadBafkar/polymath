...
2. Phase 0 — loader gains only the `project.local.yaml` overlay (existing layers byte-identical), per-key `x-merge` policy, schema keys (`conventions_docs`, `smoke`, `tracker`, `routing`, `attribution`, `artifact_matrix`; opened `prompts`), KNOWN_TOP_KEYS derived from schema + warn-not-fail unknown keys, delete dead `mcp_servers`, `${project.*:-fallback}` placeholders. — Status: not started
