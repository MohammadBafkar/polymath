...
| `polymath-connector-tracker`    | Wraps official Jira + Linear MCP servers                            | Triage workflows + provider-agnostic file-bug-from-incident; one `issue_tracker` capability  | Demote a provider if its official MCP ships triage automation covering our flow.           |
